...
void initEyes(void)
{
  Serial.println("Initialize eye objects");
  ...
  pinMode(BLINK_PIN, INPUT_PULLUP); // Same for all-eyes blink pin
#endif
}
    ...
    for (screenX = 0; screenX < SCREEN_WIDTH; screenX++, scleraX++, irisX++, lidX += dlidX) {
        ...
        if (d < IRIS_MAP_HEIGHT) {                      // Within iris area
          ...
          switch ( pressCount) {
            case 0:
              ...
              break;
            case 1:
              p = pgm_read_word(iris_1 + d * IRIS_MAP_WIDTH + a);                           // Pixel = iris
              break;
            case 2:
              p = pgm_read_word(iris_2 + d * IRIS_MAP_WIDTH + a);                           // Pixel = iris
              break;              
            case 3:
              p = pgm_read_word(iris_3 + d * IRIS_MAP_WIDTH + a);                           // Pixel = iris
              break;              
            case 4:
              p = pgm_read_word(iris_4 + d * IRIS_MAP_WIDTH + a);                           // Pixel = iris
              break;   
            case 5:
              p = pgm_read_word(iris_4 + d * IRIS_MAP_WIDTH + a);                           // Pixel = iris
              break; 
      ...
      *(&pbuffer[dmaBuf][0] + pixels++) = p >> 8 | p << 8;
      ...
      if (pixels >= BUFFER_SIZE) {
        // Check mode switching during pixel buffer refresh
        if (quickModeCheck()) {
          // If mode changes, immediately end current frame rendering
          tft.endWrite();
...
// Quick mode detection function: Check if need to immediately exit current mode
bool quickModeCheck() {
  char newMode;
  ...
  return (newMode != CUR_MODE);  // Return true indicates mode has changed
}
...
void frame(uint16_t iScale) // Iris scale (0-1023)
{
  ...
  // Perform mode detection at the beginning of frame function, exit immediately if mode changes
  if (quickModeCheck()) {
    return;  // Exit immediately, let main loop handle mode switching
  }

  if (!(++frames & 1023)) { // Print FPS only every 1024 frames (reduce serial output frequency)
  ...
  if (CUR_MODE != 'a') {
    ...
    if (CUR_MODE == 'm') {
      // Check mode switching before reading joystick data
      if (quickModeCheck()) {
      ...
      // Mode c is now dedicated to GIF playback, no need to handle eye position
      return; // Return directly, no eye rendering
    }

    // Reduce serial output frequency: print coordinates every 16 frames (originally every frame)
    static uint32_t serialFrameCount = 0;
    if (CUR_MODE == 'm' && !(++serialFrameCount & 15)) { // Print every 16 frames
      Serial.print("eyeX = ");
      Serial.print(eyeX);
      Serial.print("    eyeY = ");
      Serial.println(eyeY);
...
//    if (eyeX < 0) eyeX =  1023 / 2 - 150;
//    if (eyeY < 0) eyeY =  1023 / 2 - 100;
//#ifdef JOYSTICK_X_FLIP
//    eyeX = 1023 - eyeX -150;
//#endif
//#ifdef JOYSTICK_Y_FLIP
...
//#endif
  ...
  if ((t - timeOfLastBlink) >= timeToNextBlink) { // Start new blink?
    ...
    for (uint8_t e = 0; e < NUM_EYES; e++) {
      if (CUR_MODE!='m' && eye[e].blink.state == NOBLINK) {
  ...
  eyeX = map(eyeX, 0, 1023, 0, SCLERA_WIDTH  - 220);
  eyeY = map(eyeY, 0, 1023, 0, SCLERA_HEIGHT - 200);
  ...
  if (eyeX > (SCLERA_WIDTH - 220)) eyeX = (SCLERA_WIDTH - 220);
  ...
  //eyeX decreases to look right, increases to look left
//eyeY decreases to look down, increases to look up
  drawEye(eyeIndex, iScale, eyeX, eyeY, n, lThreshold);
...
void split( // Subdivides motion path into two sub-paths w/randimization
  ...
  if (range >= 8) {    // Limit subdvision count, because recursion
    ...
    while ((dt = (micros() - startTime)) < duration) {
          // Check mode switching during auto mode movement
    if (quickModeCheck()) {
      return;  // If mode changes, exit immediately
    }
